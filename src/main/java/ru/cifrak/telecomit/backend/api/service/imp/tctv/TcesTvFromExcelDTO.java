...
public class TcesTvFromExcelDTO implements TcesTvDTOFromExcel {

    private Workbook book;

    private final MultipartFile file;

    public TcesTvFromExcelDTO(MultipartFile file) {
        this.file = file;
    }
    ...
    @Override
    public List<TcTvFromExcelDTO> getTcesTvDTO() {
        setBook(file);
        List<TcTvFromExcelDTO> tces = new ArrayList<>();
        ...
        for (int i = 2; i < sheet.getPhysicalNumberOfRows(); i++) {
            ...
            if (row != null) {
                ...
                if (this.notEmptyRow(row)) {
                    tces.add(new TcTvFromExcelDTO(row));
                }
            }
        }
        return tces;
