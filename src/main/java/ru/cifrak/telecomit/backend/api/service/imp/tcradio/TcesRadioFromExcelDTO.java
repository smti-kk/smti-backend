...
public class TcesRadioFromExcelDTO implements TcesRadioDTOFromExcel {

    private Workbook book;

    private final MultipartFile file;

    public TcesRadioFromExcelDTO(MultipartFile file) {
        this.file = file;
    }
    ...
    @Override
    public List<TcRadioFromExcelDTO> getTcesDTO() {
        setBook(file);
        List<TcRadioFromExcelDTO> tces = new ArrayList<>();
        ...
        for (int i = 2; i < sheet.getPhysicalNumberOfRows(); i++) {
            ...
            if (row != null) {
                ...
                if (this.notEmptyRow(row)) {
                    tces.add(new TcRadioFromExcelDTO(row));
                }
            }
        }
        return tces;
