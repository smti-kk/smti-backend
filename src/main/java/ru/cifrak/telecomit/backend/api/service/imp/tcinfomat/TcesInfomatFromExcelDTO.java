...
public class TcesInfomatFromExcelDTO implements TcesInfomatDTOFromExcel {

    private Workbook book;

    private final MultipartFile file;

    public TcesInfomatFromExcelDTO(MultipartFile file) {
        this.file = file;
    }
    ...
    @Override
    public List<TcInfomatFromExcelDTO> getTcesDTO() {
        setBook(file);
        List<TcInfomatFromExcelDTO> tces = new ArrayList<>();
        ...
        for (int i = 2; i < sheet.getPhysicalNumberOfRows(); i++) {
            ...
            if (row != null) {
                ...
                if (this.notEmptyRow(row)) {
                    tces.add(new TcInfomatFromExcelDTO(row));
                }
            }
        }
        return tces;
