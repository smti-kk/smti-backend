...
public class TcesInternetFromExcelDTO implements TcesInternetDTOFromExcel {

    private Workbook book;

    private final MultipartFile file;

    public TcesInternetFromExcelDTO(MultipartFile file) {
        this.file = file;
    }
    ...
    @Override
    public List<TcInternetFromExcelDTO> getTcesInternetDTO() {
        setBook(file);
        List<TcInternetFromExcelDTO> tces = new ArrayList<>();
        ...
        for (int i = 2; i < sheet.getPhysicalNumberOfRows(); i++) {
            ...
            if (row != null) {
                ...
                if (this.notEmptyRow(row)) {
                    tces.add(new TcInternetFromExcelDTO(row));
                }
            }
        }
        return tces;
